                ...
                <div className="flex justify-between items-center h-14 sm:h-16">
                    ...
                    <div className="md:hidden">
                        <button
                            type="button"
                            onClick={toggleMenu}
                            className="bg-gray-800/80 backdrop-blur-md border border-gray-600 rounded-lg p-2 sm:p-2.5 inline-flex items-center justify-center text-white hover:text-cyan-400 hover:bg-cyan-400/20 hover:border-cyan-400 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-cyan-400/50 focus:ring-offset-2 focus:ring-offset-black shadow-lg"
                            {...(isMenuOpen && { 'aria-expanded': true })}
                        >
                            <span className="sr-only">Open main menu</span>
                            {/* Hamburger icon with improved animation */}
                            <svg
                                className={`h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300 ease-in-out ${isMenuOpen ? 'rotate-90' : 'rotate-0'}`}
